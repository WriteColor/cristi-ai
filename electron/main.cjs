...
// Optional Windows Core Audio loopback transport. Chromium's getDisplayMedia
// can expose system audio only for sources the browser allows the user to
// select; this transport provides a deterministic default-render endpoint for
// game/system audio while keeping all capture in the privileged main process.
let nativeDesktopAudioState = null;

function resolveWasapiHelperPath() {
  const candidates = app.isPackaged
    ...
    : [path.join(__dirname, '..', 'native', 'CristiWasapiLoopback.exe')];
  return candidates.find(candidate => fs.existsSync(candidate)) || null;
}

function emitNativeDesktopAudioEvent(payload, state = nativeDesktopAudioState) {
  const sender = state?.sender;
  if (sender && !sender.isDestroyed()) {
    try { sender.send('desktop-audio-native-event', payload); } catch (_) {}
  }
}

function stopNativeDesktopAudioCapture(reason = 'stopped') {
  const state = nativeDesktopAudioState;
  nativeDesktopAudioState = null;
  if (!state) return { success: true, alreadyStopped: true };
  state.stopping = true;
  activeChildProcesses.delete(state.child);
...
// ── Native Windows system-audio loopback (WASAPI) ────────────────────────────
ipcMain.handle('desktop-audio-native-start', async (event, { sourceId = 'system_loopback' } = {}) => {
  const requestedSourceId = String(sourceId || 'system_loopback').trim().slice(0, 120) || 'system_loopback';
  ...
  const helperPath = resolveWasapiHelperPath();
  if (!helperPath) {
    return {
      success: false,
      available: false,
      error: 'El helper WASAPI no está incluido en este paquete. Usa la captura de pantalla o reconstruye el instalador.'
    };
  }
  ...
  try {
    ...
    return { success: false, available: false, error: `No se pudo iniciar WASAPI: ${error.message}` };
  }
  ...
  child.stdout.on('data', chunk => {
    ...
    // Frames have a fixed 16-byte header: CRIS, sample rate, payload bytes,
    // flags, followed by little-endian PCM16 payload.
    while (state.buffer.length >= 16) {
      ...
      if (magicOffset > 0) state.buffer = state.buffer.subarray(magicOffset);
...
ipcMain.handle('desktop-audio-native-status', () => {
  const state = nativeDesktopAudioState;
  return {
...
// ── Persistent memory storage (SQLite when available, atomic JSON fallback) ─
...
function getMemoryDatabase() {
  ...
  try {
    ...
    memoryDatabase = new DatabaseSync(dbPath);
    memoryDatabase.exec(`
    ...
    console.warn('[Memory] SQLite no disponible; usando JSON atómico:', error?.message || String(error));
    return null;
  }
}

function memoryJsonPath() {
...
ipcMain.handle('memory-save', async (event, memories) => {
  const records = Array.isArray(memories) ? memories.slice(0, 20000) : [];
  const db = getMemoryDatabase();
  if (db) {
    try {
      db.exec('BEGIN');
      ...
      console.warn('[Memory] Error guardando SQLite; usando JSON:', error?.message || String(error));
    }
  }
