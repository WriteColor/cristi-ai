...
if (typeof window !== 'undefined') {
  ...
  if (PIXI.DisplayObject && !PIXI.DisplayObject.prototype.isInteractive) {
    PIXI.DisplayObject.prototype.isInteractive = function () { return false; };
  }
  if (Live2DModel?.prototype && !Live2DModel.prototype.isInteractive) {
    Live2DModel.prototype.isInteractive = function () { return false; };
  }
}
...
function loadModel(path, isCurrent) {
  const loading = modelLoadQueue.then(async () => {
    if (!isCurrent()) return null;
    const loaded = await Live2DModel.from(path, { autoInteract: false, autoUpdate: false });
    if (!isCurrent()) {
...
export const Live2DCanvas = React.memo(forwardRef(function Live2DCanvas(
  ...
  // ── DOM Refs ───────────────────────────────────────────────────────────────
  const containerRef  = useRef(null); // Contiene el canvas WebGL
  const hitTargetRef  = useRef(null); // Capa interactiva (bounding box del modelo)
  const wrapperRef    = useRef(null); // Div raíz, pointer-events: none
  ...
  // Los límites de model.getBounds() en PIXI con autoDensity: true corresponden
  // 1:1 a las coordenadas CSS de pantalla.
  ...
  useEffect(() => {
    ...
    async function init() {
      try {
        ...
        // Limitar a 60 FPS estrictos para no saturar monitores de 144Hz/240Hz y permitir que juegos y videos corran sin tirones
        if (app.ticker) {
          app.ticker.maxFPS = 60;
        }

        if (app.view) {
          // CRÍTICO: el canvas NUNCA captura eventos de puntero
        ...
        // Super-optimización: Filtrado bilineal de alta fidelidad sin blur en texturas
        ...
        // 6. Conectar adaptador y controlador de expresiones
        ...
        // 7. Conectar bucle de animación y cinética (Físicas, Respiración y Lip-Sync a 60 FPS)
        const onTick = (delta) => {
          if (!isMounted) return;
        ...
        // One clock drives motion, physics and rendering; no shared ticker race.
        app.ticker.add(onTick);

        // 8. Aplicar layout y cebar WebGL con dos pases de render
        applyLayout(model, app, viewModeRef.current);
        model.update(16);
        app.renderer.render(app.stage);

        // Segundo pase tras montar para garantizar texturas correctas
        requestAnimationFrame(() => {
          ...
          syncHitTargetBounds();
          setIsLoading(false);
          setModelReady(true); // ← activa pointer-events del hitTarget
        });

        // 9. Exponer API para testing externo
        if (typeof window !== 'undefined') {
          window.__cristiAvatar = {
            ...
            setExpression:   (exp) => adapterRef.current?.setExpression(exp),
            setEmotion:      (emo) => controllerRef.current?.setEmotion(emo)
          };
        }

      } catch (err) {
        logger.error('Live2D', `Error fatal: ${err.message}`, err);
        if (isMounted) {
  ...
  // ── Optimización Dinámica de Rendimiento en Juegos ────────────────────────
  // Reduce el Ticker a 30 FPS cuando otra aplicación o juego tiene el foco,
  // liberando el 50% del uso de GPU. Restablece a 60 FPS al interactuar con Cristi.
  useEffect(() => {
    const handleBlur = () => {
      if (pixiAppRef.current?.ticker) {
        pixiAppRef.current.ticker.maxFPS = 30;
      }
    };
    const handleFocus = () => {
      if (pixiAppRef.current?.ticker) {
        pixiAppRef.current.ticker.maxFPS = 60;
      }
    };
  ...
  useImperativeHandle(ref, () => ({
    ...
    switchModel: (id) => {
      if (live2dModelRegistry.getModel(id)) {
        // No hace nada aquí — modelId cambia por prop y el useEffect re-inicia
      }
    },
    triggerMotion:  (g, i) => controllerRef.current?.playMotion?.(g, i),
    triggerGesture: (name) => controllerRef.current?.setEmotion?.(name),
  ...
  return (
     ...
     * Nunca bloquea clics al resto de la UI (barra de herramientas, menú contextual, etc.)
     */
    <div
      ref={wrapperRef}
    ...
    >
      {/* Canvas WebGL — también pointer-events none (solo renderiza visualmente) */}
      <div
        ref={containerRef}
      ...
      */}
      <div
        ...
        style={{
          // Posición inicial fuera de pantalla hasta que syncHitTargetBounds la ajuste
          left: '-9999px',
          top: '-9999px',
          width: '100px',
          height: '100px',
          // CRÍTICO: pointer-events se controla solo por modelReady
