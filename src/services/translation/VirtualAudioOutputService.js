 ...
 * VB-CABLE Input or a VoiceMeeter virtual output. The paired capture endpoint
 * is then selected as the microphone by the game. No endpoint means no audio
 * is rendered: falling back to speakers would be surprising and can leak TTS.
 */
export class VirtualAudioOutputService {
  constructor({
    ...
    this.metrics = { played: 0, dropped: 0, failures: 0, lastError: null };
  }

  isSupported() {
    return typeof this._createContext === 'function' && typeof globalThis.AudioContext !== 'undefined'
  ...
  async configure({ deviceId = '', deviceLabel = '' } = {}) {
  ...
  async playAudioChunk(data, { sampleRate = 24000, frameId = null, sessionId = null } = {}) {
    ...
    try {
      ...
      const entry = { source, sourceId, sessionId };
  ...
  _emitLifecycle(type, frameId, sessionId) {
