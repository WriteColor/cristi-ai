...
| MCP, Spotify y navegador | `MCPClientManager`, Spotify local y Playwright Brave | Transporte real y ciclo de vida | MCP `stdio` JSON-RPC, Brave executable, IPC Electron | Un servidor remoto necesita autenticación y políticas propias; nunca exponer `spawn` al renderer | Ejecutar MCP en main, namespace por servidor, validar esquema y enrutar `tools/call`; añadir HTTP/SSE como fase posterior |
