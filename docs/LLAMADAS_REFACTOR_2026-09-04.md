...
- **Subtítulos:** la transcripción de Ariel se actualiza con el primer delta recibido y permanece visible durante turnos largos. Los deltas de texto/transcripción de Cristi se acumulan fuera de la interfaz y se publican como una única respuesta completa al recibir `turnComplete`, evitando que el subtítulo se adelante a su voz. Ambos paneles admiten respuestas extensas con desplazamiento y un área de hasta el 70 % de la ventana.
...
- **Gemini Live:** todos los modelos envían texto y frames por `realtimeInput`, manteniendo un orden único de medios. Un cierre transitorio conserva el último turno pendiente y lo reintenta una sola vez si no existe un identificador de reanudación, sin repetir respuestas ya aceptadas por el servidor. Gemini 3.1 usa `thinkingLevel`; Gemini 2.5 conserva `thinkingBudget`.
...
| API real Gemini 2.5 | El probe de voz entregó audio y transcripción; las pruebas de visión pueden recibir cierres 1011 de disponibilidad del proveedor, que ahora activan reconexión y reintento acotado |
...
Los subtítulos de Ariel aparecen al recibir el primer delta de transcripción de Gemini. El subtítulo de Cristi se publica completo al cerrar el turno para no adelantarse a la voz, aunque el servidor entregue la transcripción por partes. La transcripción automática puede equivocarse: Gemini 2.5 no reprodujo exactamente la frase sintética de entrada. Un código 1011 de Gemini indica indisponibilidad temporal del servicio; el socket conserva el turno y ejecuta como máximo un reintento sin duplicar una sesión reanudable.
